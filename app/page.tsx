  ...
  return <main className="app-shell">
    ...
    <div className="workspace">
      ...
      <section className="detail-panel">{selected && <>
        ...
        {tab === "code" ? <div className="editor-wrap"><div className="editor-bar"><span>{codeFileName(selected.title, selected.language)}</span><span className="editor-language">{selected.language}</span><span className="traffic"><i /><i /><i /></span></div><CodeEditor value={selected.code} onValueChange={(code) => updateSelected({ code })} highlight={(code) => highlightCode(code, selected.language)} /></div> : <div className="notes"><label htmlFor="algorithm-notes">使いどころ・注意点</label><textarea id="algorithm-notes" value={selected.description} onChange={(e) => updateSelected({ description: e.target.value })} /><label htmlFor="algorithm-tags">タグ</label><input id="algorithm-tags" value={selected.tags.join(", ")} onChange={(e) => updateSelected({ tags: e.target.value.split(",").map((tag) => tag.trim()).filter(Boolean) })} /></div>}
